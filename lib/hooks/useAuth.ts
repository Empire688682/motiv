...
export const useAuth = () => {
  ...
  const [hasToken, setHasToken] = useState<boolean | null>(null); // null = checking, true/false = determined
  ...
  const {
    ...
    enabled: isClient && hasToken === true, // Only run if we're on client and have token
  ...
  return {
    // Data
    user,
    isLoading: !isClient || hasToken === null || (hasToken && isLoading),
    error,
