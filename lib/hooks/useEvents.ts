...
// Get similar events
export const useSimilarEvents = (eventId: string) => {
  return useQuery({
    queryKey: ['events', 'similar', eventId],
    queryFn: () => eventService.getSimilarEvents(eventId),
    enabled: !!eventId,
    staleTime: 5 * 60 * 1000, // 5 minutes
  });
};

// Get popular events (shuffled)
export const usePopularEvents = (limit: number = 6) => {
  return useQuery({
    queryKey: ['events', 'popular', limit],
    queryFn: () => eventService.getPopularEvents(limit),
    staleTime: 0, // Always fetch fresh data for shuffled results
    gcTime: 1 * 60 * 1000, // Keep in cache for 1 minute but always refetch
  });
};
