...
export function AuthModal({ isOpen, onClose, onAuthSuccess }: AuthModalProps) {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData((prev) => ({
      ...
      // If exclusive is checked, also set newsletterSubscribed to true
      ...(name === "exclusive" && type === "checkbox" && checked ? { newsletterSubscribed: true } : {}),
    }));
  };
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        // If exclusive checkbox was checked during login, update newsletter subscription
        if (formData.exclusive) {
          try {
            // Update user profile to set newsletter subscription
