import { Button } from "@/components/ui/button"
import Image from "next/image"

export function HeroSection() {
  return (
    <section className="px-4 md:px-6 py-6 md:py-8 relative overflow-hidden">
      {/* Subtle background gradient */}
      <div className="absolute inset-0 bg-gradient-to-br from-[#D72638]/5 via-transparent to-transparent pointer-events-none"></div>
      
      <div className="grid lg:grid-cols-2 gap-8 items-center max-w-7xl mx-auto relative">
        <div className="space-y-6">
          <h1 className="text-4xl hidden md:block md:text-6xl lg:text-7xl font-bold leading-tight font-geist bg-gradient-to-r from-white via-gray-100 to-gray-300 bg-clip-text text-transparent animate-pulse-subtle">
            DISCOVER
            <br />
            LAGOS
            <br />
            EVENTS
          </h1>
          <h1 className="text-4xl md:text-6xl md:hidden lg:text-7xl font-bold leading-tight font-geist bg-gradient-to-r from-white via-gray-100 to-gray-300 bg-clip-text text-transparent animate-pulse-subtle">
            DISCOVER
            LAGOS
            EVENTS
          </h1>
          <p className="text-gray-300 text-lg md:text-xl max-w-md leading-relaxed">
            Unfiltered Lagos events. Curated events, no tickets lost, just vibes.
          </p>
          <a href="#events-section">
          <Button className="mt-4 bg-gradient-to-r from-[#D72638] to-[#B91E2F] hover:from-[#B91E2F] hover:to-[#A01A2A] text-white px-8 py-3 text-lg rounded-full shadow-lg hover:shadow-[#D72638]/30 hover:shadow-2xl transition-all duration-300 hover:scale-105 group">
            <span className="group-hover:translate-x-1 transition-transform duration-300">Get Started</span>
          </Button>
          </a>
        </div>

        <div className="relative group">
          <Image
            src="https://res.cloudinary.com/dicg57iox/image/upload/v1758363125/ato-aikins-a6CrotZc7R4-unsplash_t6b7b7.png"
            alt="Event party with vibrant lighting and crowd"
            width={600}
            height={400}
            className="rounded-lg object-cover w-full shadow-xl group-hover:scale-102 transition-transform duration-300"
          />
        </div>
      </div>
    </section>
  )
}