...
interface TicketCardProps {
  ticket: {
    ...
    qrCode: string;
    qrData?: string; // Raw QR code data string
    attendeeData: {
      fullName: string;
      email: string;
      phone: string;
...
export function TicketCard({ ticket }: TicketCardProps) {
  ...
  useEffect(() => {
    // Check if QR code is already a base64 image or needs to be generated
    if (ticket.qrCode.startsWith('data:image/') || ticket.qrCode.length > 100) {
      // Already a base64 image or base64 encoded data
      if (ticket.qrCode.startsWith('data:image/')) {
        setQrCodeUrl(ticket.qrCode);
      } else {
        // Base64 encoded PNG data, add data URL prefix
        setQrCodeUrl(`data:image/png;base64,${ticket.qrCode}`);
      }
    } else {
              ...
              <div className="space-y-2 text-xs sm:text-sm">
                ...
                {ticket.qrData && (
                  <div className="pt-2 border-t border-gray-200">
                    <span className="text-gray-600 text-xs block mb-1">QR Code Data:</span>
